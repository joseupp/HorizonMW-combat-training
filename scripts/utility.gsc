...
createTriggerVisionSet( visionFile, transitionTime, origin )
{
	level endon( "game_ended" );

	trigger = spawn( "trigger_radius", origin, 0, 100, 20 );
	...
	while( 1 )
	{
		...
		if( isDefined( player ) && isPlayer( player ) && (!isDefined(player.visionSet) || player.visionSet != visionFile) )
		{
			player.visionSet = visionFile;
			player visionSetNakedForPlayer( visionFile, transitionTime );

			if( isDefined( level.vision_set_fog[visionFile] ) )
				player set_fog_to_ent_values( level.vision_set_fog[visionFile], transitionTime );
		}
	}
}
...
show_hint_string(text, endonString1, endonString2, endonString3)
{
    ...
    {
        self.hint_string = self maps\mp\gametypes\_hud_util::createFontString("default", 1.5);
...
array_combine_ex(array_1, array_2, array_3, array_4, array_5)
{
    combined_array = [];

    if (isdefined(array_1) && array_1.size > 0)
    {
        foreach (item in array_1)
        {
            if (isdefined(item))
                combined_array[combined_array.size] = item;
        }
    }

    if (isdefined(array_2) && array_2.size > 0)
    {
        foreach (item in array_2)
        {
            if (isdefined(item))
                combined_array[combined_array.size] = item;
        }
    ...
    if (isdefined(array_3) && array_3.size > 0)
    {
        foreach (item in array_3)
        {
            if (isdefined(item))
                combined_array[combined_array.size] = item;
        }
    ...
    if (isdefined(array_4) && array_4.size > 0)
    {
        foreach (item in array_4)
        {
            if (isdefined(item))
                combined_array[combined_array.size] = item;
        }
    ...
    if (isdefined(array_5) && array_5.size > 0)
    {
        foreach (item in array_5)
        {
            if (isdefined(item))
                combined_array[combined_array.size] = item;
        }
