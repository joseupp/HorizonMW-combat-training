...
local function DisplayVoiceModerationMessage()
    local heightOffset = 58;
    local display_width = 450

    local element = LUI.UIElement.new({
        rightAnchor = true,
        topAnchor = true,
        bottom = heightOffset + 20,
        width = display_width,
        height = 20,
    ...
    text:setText("Voice chat is NOT recorded and NOT moderated. Have fun!")
...
LUI.MenuBuilder.m_types_build["teamScoresHudDef"] = function()
	...
	if Engine.GetDvarString("g_gametype") == "dz" then barLeft = scoreTextLeft + 79.5 end
    ...
    local voiceNotification = DisplayVoiceModerationMessage()
    root:addElement(voiceNotification)
    voiceNotification:hide()

    root:registerEventHandler("showvoicemessage", function()
        voiceNotification:show()
    end)

    root:registerEventHandler("hidevoicemessage", function()
        voiceNotification:hide()
    end)
