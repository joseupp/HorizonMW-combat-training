...
local has_material_tag = function ( bytes )
	...
	local sm2 = test_tag( "sm", 0x4D, 0x30, bytes )
	if sm2 ~= nil then
		return true, 60
	end

	local hmw = test_tag( "hm", 0x4D, 0x30, bytes )
	if hmw ~= nil then
		return true, 60
	end

	local soar = test_tag( "sr", 0x40, 0x40, bytes )
	if soar ~= nil then
		return true, 60
	end

	local faze = test_tag( "fz", 0x40, 0x40, bytes )
	if faze ~= nil then
		return true, 60
	end
