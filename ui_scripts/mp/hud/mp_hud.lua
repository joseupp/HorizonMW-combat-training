...
MPHud.initWidgets = function ( f15_arg0 )
	...
	if not Engine.InFrontend() then
		...
		f15_arg0.sentryHud = MPHud.addWidget( f15_arg0, LUI.MenuBuilder.buildItems( {
			type = "sentry_overlay",
			properties = {
				enabledAlpha = 1
			}
		...
		f15_arg0.weapAttachment = MPHud.addWidget( f15_arg0, LUI.MenuBuilder.buildItems( {
			type = "weaponAttachmentProcessing"
		} ), {
			scalable = true
		} )
		if Engine.GetDvarString("ui_gametype") == "dz" then
			f15_arg0.dz_hud_id = MPHud.addWidget( f15_arg0, LUI.MenuBuilder.buildItems( {
				type = "dz_hud_def"
			} ), {
				shakeable = true,
				scalable = true
