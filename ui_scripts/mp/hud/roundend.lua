...
local ReasonStrings = {"MP_SCORE_LIMIT_REACHED", "MP_TIME_LIMIT_REACHED", "MP_PLAYERS_FORFEITED", "MP_TARGET_DESTROYED",
                       "MP_BOMB_DEFUSED", "MP_ALLIES_ELIMINATED", "MP_AXIS_ELIMINATED", "MP_ALLIES_FORFEITED",
                       "MP_AXIS_FORFEITED", "MP_SAS_MISSION_ACCOMPLISHED", "MP_SPETSNAZ_MISSION_ACCOMPLISHED",
...
local placementStrings = {"1st", "2nd", "3rd"}

local function PlayerItem( player, index )
    local font = CoD.TextSettings.TitleFont50
    local fontAlpha = 1
    local fontColour = index == 1 and Colors.mw1_green or Colors.white
    local fontScale = 24 - (3 * index)
    local frameStyle = index == 1 and LUI.DecoFrame.Highlight or LUI.DecoFrame.None
    ...
    root:registerAnimationState("inactive", {
        top = 40,
        alpha = 0
    })
    ...
    local backgroundList = LUI.UIHorizontalList.new({
        height = 40,
        width = width,
    ...
    local function PlayerFrame( _width_, _add_stroke_ )
        local root = LUI.UIElement.new({
            height = 40,
            width = _width_
        })
        local placebackground = LUI.UIImage.new({
        ...
        root:addElement(placebackground)
        return root
    end
    
    local placebackground = PlayerFrame( scoreWidth, false )
    ...
    placebackground:addElement(placement)

    local namebackground = PlayerFrame( width - scoreWidth * 2, false )
    local playerName = LUI.UIText.new({
        rightAnchor = true,
        font = CoD.TextSettings.TitleFontMedium.Font,
        alignment = LUI.Alignment.Left,
        width = width - scoreWidth * 2,
        height = fontScale,
        color = fontColour,
        alpha = fontAlpha
    })
    playerName.id = "player_name_" .. index
    playerName:setText("   " .. player.name)
    playerName:setUsePulseFX(true, false)
    namebackground:addElement(playerName)

    local scorebackground = PlayerFrame( scoreWidth, true )
    ...
    playerScore.id = "player_score_" .. index
    playerScore:setText(player.score)
    playerScore:setUsePulseFX(true, false)
    scorebackground:addElement(playerScore)

    backgroundList:addElement(placebackground)
    backgroundList:addElement(namebackground)
    backgroundList:addElement(scorebackground)
    if index == 1 then
        local frame = LUI.DecoFrame.new({
            rightAnchor = true,
            right = -10,
            height = 45,
            width = width + 14
        ...
        frame:addElement(decor)
        root:addElement(frame)
    end

    root:addElement(backgroundList)
    root:animateToState("inactive")
    return root
end
...
function RoundEndSlayer()
    ...
    for index = 1, #placementStrings, 1 do
        ...
        if player and player.name then
            ...
            local element = PlayerItem(player, index)
            playerList:addElement(element)
            element:animateToState("active", 2000)
        end
    end
    return mainElement
end
