-- MAKE SURE YOU ADD NEW TABS to h2m_files.csv otherwise it won't work
-- When adding a new tab make sure to require it here
require("h1_tab")
require("iw4_tab")
require("iw5_tab")
require("h2_tab")

-- When adding a new tab require its map data here
local h1_maps = require("data.h1_maps")
local iw4_maps = require("data.iw4_maps")
local iw5_maps = require("data.iw5_maps")
--local custom_maps = require("data.h2_maps")

-- When adding a new tab, add a var here
local TAB_H1 = "h1";
local TAB_IW4 = "iw4";
local TAB_IW5 = "iw5";
local TAB_CUSTOM = "h2";

local currentTab = TAB_H1
local number_of_tabs = 3; -- When adding a new tab increment this

-- When adding a new tab return it here
local function GetMaps()
    --if currentTab == TAB_CUSTOM then
    --    return custom_maps
    if currentTab == TAB_IW4 then
        return iw4_maps
    elseif currentTab == TAB_IW5 then
        return iw5_maps
    elseif currentTab == TAB_H1 then
        return h1_maps
    else
        return nil
    end
...
-- When adding a new tab,return its controller idx here
local function GetMapData(controller)
    --if controller.game == TAB_CUSTOM then
    --    return custom_maps[controller.idx]
    if controller.game == TAB_IW4 then
        return iw4_maps[controller.idx]
    elseif controller.game == TAB_IW5 then
        return iw5_maps[controller.idx]
    elseif controller.game == TAB_H1 then
        return h1_maps[controller.idx]
    else
        return nil
    end
end
...
LUI.MapSetup.AddTabs = function ( menu )
    ...
    local IW5Maps = LUI.MenuBuilder.BuildRegisteredType( "iw5_tab", {
        controller = menu.controller
    } )
    tabs[#tabs + 1] = IW5Maps
    menu:addElement( IW5Maps )
...
local function SelectMap(menu, controller)
    local mapData = GetMapData(controller)
