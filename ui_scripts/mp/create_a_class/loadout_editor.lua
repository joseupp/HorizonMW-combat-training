...
local MeleeInUseCheck = function()
    ...
    for melee = 1, #melees, 1 do
        if weapon_in_use == melees[melee] then
            return true --switched from false to true (CnR)
        end
    end

    return false --switched from true to false (CnR)
end
...
local MeleeOrLauncherInUseCheck = function() --simplified the function (CnR)
    return MeleeInUseCheck() or LauncherInUseCheck()
end

local BlingProPerkCheck = function() --New Bling Pro check (CnR)
    return Cac.HasEquippedWeapon("Perk_Slot1", "specialty_secondarybling", LUI.CacStaticLayout.ClassLoc)
end

Cac.BlingPerkCheck = function() --new Bling check (CnR)
    return Cac.HasEquippedWeapon("Perk_Slot1", "specialty_bling", LUI.CacStaticLayout.ClassLoc) or BlingProPerkCheck()
end
...
local Categories = {{
    ...
    options = {{
        ...
        attributesDelta = true,
        showUnlocks = false,
        visibleFunc = function() --added this (CnR)
            return not MeleeOrLauncherInUseCheck()
        end
    }, {
        name = "@LUA_MENU_ATTACHMENT_CAPS",
        ...
        attributesDelta = true,
        showUnlocks = false,
        visibleFunc = function() --added this cancer (CnR)
            return not MeleeOrLauncherInUseCheck() and Cac.BlingPerkCheck()
        end
    }, {
        name = "@MENU_CAMO_SHORT_CAPS",
        ...
        cacLootTabs = CamoTabs,
        visibleFunc = function() --added this (CnR)
            return not MeleeInUseCheck() and not LauncherInUseCheck()
        end
    }, {
        name = "@MENU_CAMO_SHORT_CAPS", --added second category for melee camo slot (CnR)
        storageCategory = "Secondary_Camo",
        weaponCategory = "camo",
        showUnlocks = false,
        cacLootTabs = CamoTabs,
        visibleFunc = function()
            return MeleeInUseCheck()
        end
    }},
    showAttributes = true,
...
local function build_extra_helps(f18_local1)
    local controller   = Cac.GetSelectedControllerIndex()
    local class_index  = Cac.GetSelectedClassIndex(LUI.CacStaticLayout.ClassLoc)

    local isSniper = Cac.GetWeaponTypeFromWeaponWithoutCategory(
        Cac.GetEquippedWeapon("Primary", 0, LUI.CacStaticLayout.ClassLoc)
    ) == "weapon_sniper"

    if isSniper then
        local use2d = Cac.GetCacConfig(controller, LUI.CacStaticLayout.ClassLoc, class_index, "use2dScopes")
        local helper_text = use2d and "Disable 2D Scopes" or "Enable 2D Scopes"
        ...
        f18_local1:AddHelp({
            ...
            priority = 0,
            func = function()
                local prev = Cac.GetCacConfig(controller, LUI.CacStaticLayout.ClassLoc, class_index, "use2dScopes")
                Cac.SetCacConfig(controller, LUI.CacStaticLayout.ClassLoc, class_index, "use2dScopes", not prev)

                if (not prev) == true then
                    local acog = hasAcogScope()
                    local therm = hasThermalScope()
                    if acog[1] == true then
                        remove_weapon_config(acog[2], 0, LUI.CacStaticLayout.ClassLoc, nil)
                    elseif therm[1] == true then
                        remove_weapon_config(therm[2], 0, LUI.CacStaticLayout.ClassLoc, nil)
                    end
                    RefreshAttachments()
                end
                build_extra_helps(f18_local1)
            end
        }, nil, nil, true)
    end

    local has_soh = Cac.GetEquippedWeapon("Perk_Slot1", 0, LUI.CacStaticLayout.ClassLoc) == "specialty_quickswap"
    if has_soh then
        local iw4_soh = Cac.GetCacConfig(controller, LUI.CacStaticLayout.ClassLoc, class_index, "use_iw4_soh")
        local helper_text = iw4_soh and "Enable SoH Pro" or "Disable SoH Pro (no fast swap)"
        ...
        f18_local1:AddHelp({
            ...
            priority = 0,
            func = function()
                local prev = Cac.GetCacConfig(controller, LUI.CacStaticLayout.ClassLoc, class_index, "use_iw4_soh")
                Cac.SetCacConfig(controller, LUI.CacStaticLayout.ClassLoc, class_index, "use_iw4_soh", not prev)
                build_extra_helps(f18_local1)
            end
        }, nil, nil, true)
    end
